...
function getVideoId(querystring) {
    ...
    if (window.location.hash && window.location.host.indexOf("vimeo.com") > -1 ) {
    ...
    // Embeded and vimeo.com without #
    video_id = querystring.substr(querystring.lastIndexOf("/") + 1);
    if ( video_id.indexOf("?") > -1 ) {
	video_id = video_id.substr(0, video_id.indexOf("?"));
    }
    return video_id;
...
function updateXBMCMenu(a) { // Fills but doesn't expand.
...
function addButtonWithMenu() {
    var menu = document.getElementById("clip_description_link");
    if ( !menu) {
	var menu = document.getElementById("description");
	if ( menu ) {
    ...
    if ( menu ) {
	var xbmc_test = '<li><button type="button" id="xbmc-play-button" class="start yt-uix-tooltip-reverse addto-button-plus-hide-arrow yt-uix-button yt-uix-tooltip" onclick="" title="Play in XBMC" data-button-menu-id="" data-button-action="" role="button" aria-pressed="false"><img class="yt-uix-button-arrow" src="//s.ytimg.com/yt/img/pixel-vfl3z5WfW.gif" alt=""></button><button type="button" class="end yt-uix-tooltip-reverse yt-uix-button yt-uix-tooltip yt-uix-button-empty" onclick=";return false;" title="More XBMC Functions" data-button-menu-id="shared-xbmc-menu" data-button-action="" role="button" aria-pressed="false"><img class="yt-uix-button-arrow" src="//s.ytimg.com/yt/img/pixel-vfl3z5WfW.gif" alt=""></button><div id="shared-xbmc-menu" style="display: none;"><ul class="xbmc-menu"></ul></li>';
...
function run() {
    if ( window.location.host.indexOf("vimeo.com") > -1 ) {
	if ( xbmc_autoplay ) {
	    playVideo(false);
	}
	...
	if (document.getElementById("clip_description_link") ) {
	    updateXBMCMenu();
